...
<center><div id="linehome"><img src="image/line1.png"></div></center>
	...
	$id_member = $_SESSION['id_member'];
	 ...
	 $sql ="INSERT INTO order_fur (id_order,date_order,id_member,price_order) VALUES ('','{$date_order}','{$id_member}','')";
	...
	foreach ($id_product as $key => $value) {
		...
		$sql2 ="INSERT INTO order_detail (id,id_product,id_member,id_order,amount_order,totalPrice) VALUES ('','{$id_product[$key]}','{$id_member}','','{$num_cart[$key]}','{$totalPrice}')";
	...
	<?php foreach ($id_product as $key => $value)  {
	   ...
	   ?><div id="check-wrap">
	 	...
	 	ราคา : <?php echo number_format ($price_product[$key]) ?> บาท <br>
	 	จำนวน :<?php echo $num_cart[$key] ?> <br>
	 	<?php $totalPrice = $price_product[$key]*$num_cart[$key]; ?>
	 	ราคารวม : <?php echo number_format($totalPrice) ?> บาท
	 	</div>
...
<span class="to1">Total Price : <?php echo number_format($priceAll); ?> บาท</span>
   ...
   <div id="butt1">
     	...
     	<center><a href="product.php"><button id="blue-btn">เลือกซื้ออีกครั้ง</button></a></center>
